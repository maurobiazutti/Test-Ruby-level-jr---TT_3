...
def solucao(arg)
  arg.map do |item|
    # Transforma os valores em string, trunca em 11 caracteres e preenche com espaços se faltar
    name  = item[:name].to_s[0...11].ljust(11)
    cpf   = item[:cpf].to_s[0...11].ljust(11)
    state = item[:state].to_s[0...11].ljust(11)
    value = item[:value].to_s[0...11].ljust(11)
    
    # Junta os campos da linha
    puts "#{name}#{cpf}#{state}#{value}"
  end.join("\n") # Junta todas as linhas quebrando uma linha entre elas
end
